...
| W3-A | 3 | Opportunity calculator | Transparent editable illustrative opportunity model | Implemented; deployment blocked after target mismatch | Not deployed | Vercel returned `target: production` despite explicit preview target; separately authorize remediation before retry | Not accepted as published; preview-only authorization not satisfied |
