...
for (const term of ['Monthly site traffic', 'Interaction rate', 'Lead rate', 'Average revenue per sale', 'Closing rate', 'Illustrative opportunity model', 'not customer proof', 'not a forecast', 'not partner earnings', 'not a commission calculation', 'not a guaranteed result', 'Non-binding planning illustration', 'approved written terms', 'aria-describedby="input-guidance"', 'id="validation-summary"']) assert.ok(html.includes(term), `Missing ${term}`);
...
assert.match(css, /--sf-espresso:\s*#2d201b/);
